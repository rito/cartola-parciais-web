...
public class LigasTurnoFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState){
        ...
        adapter = new LigasTurnoAdapter( getActivity() , orderAdapterList(listaTimesDaLiga) );
        ...
        if (listaTimesDaLiga != null && listaTimesDaLiga.size() > 0){
            ...
            for (TimeLiga time : timesDaLiga){
                if (time.getAtletas() != null && (time.getVariacaoCartoletas() == null || time.getVariacaoCartoletas() == 0)){
                    time.setPontuacaoTurno(time.getPontuacaoTurno()+time.getPontuacao());
                }
            }

            Collections.sort(timesDaLiga, (t1, t2) -> {

                if (t1.getPontuacaoTurno() < t2.getPontuacaoTurno()) return  1;
                if (t1.getPontuacaoTurno() > t2.getPontuacaoTurno()) return -1;
