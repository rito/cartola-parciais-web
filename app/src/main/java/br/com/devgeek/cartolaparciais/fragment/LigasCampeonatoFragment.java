...
import br.com.devgeek.cartolaparciais.adapter.LigasCampeonatoAdapter;
 ...
 */
public class LigasCampeonatoFragment extends Fragment {

    private static final String TAG = "LigasCampeonatoFragment";

    private long ligaId;

    private Realm realm;
    private ApiServiceImpl apiService;
    private LigasCampeonatoAdapter adapter;
    ...
    public static LigasCampeonatoFragment newInstance(long ligaId){
        Bundle bundle = new Bundle();
        bundle.putLong("ligaId", ligaId);

        LigasCampeonatoFragment fragment = new LigasCampeonatoFragment();
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState){
        ...
        View view  = inflater.inflate(R.layout.fragment_ligastimes_campeonato, container, false);
        ...
        adapter = new LigasCampeonatoAdapter( getActivity() , orderAdapterList(listaTimesDaLiga) );
        ...
        if (listaTimesDaLiga != null && listaTimesDaLiga.size() > 0){
            ...
            for (TimeLiga time : timesDaLiga){
                if (time.getAtletas() != null && (time.getVariacaoCartoletas() == null || time.getVariacaoCartoletas() == 0)){
                    time.setPontuacaoCampeonato(time.getPontuacaoCampeonato()+time.getPontuacao());
                }
            }

            Collections.sort(timesDaLiga, (t1, t2) -> {

                if (t1.getPontuacaoCampeonato() < t2.getPontuacaoCampeonato()) return  1;
                if (t1.getPontuacaoCampeonato() > t2.getPontuacaoCampeonato()) return -1;
