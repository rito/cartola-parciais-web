 ...
 */
public class LigasCartoletasAdapter extends RecyclerView.Adapter<LigasCartoletasAdapter.ViewHolder> {

    private static final String TAG = "LigasCartoletasAdapter";

    private Context context;
    private DecimalFormat formatoCartoletas;
    private RealmResults<TimeLiga> listaTimesDaLiga;

    public LigasCartoletasAdapter(Context context, RealmResults<TimeLiga> listaTimesDaLiga){
        this.context = context;
        update(listaTimesDaLiga);
        this.formatoCartoletas = new DecimalFormat(TimeLiga.FORMATO_CARTOLETAS);
    }
    ...
    @Override
    public LigasCartoletasAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType){

        View view = LayoutInflater.from(context).inflate(R.layout.adapter_ligas_rodada, parent, false);

        return new LigasCartoletasAdapter.ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(LigasCartoletasAdapter.ViewHolder holder, int position){
        ...
        holder.setData( listaTimesDaLiga.get( position ), backgroundColor, position, listaTimesDaLiga.get( 0 ).getPatrimonio());
    }
    ...
    class ViewHolder extends RecyclerView.ViewHolder {
        ...
        private void setData(TimeLiga time , int backgroundColor, int position, double cartoletasPrimeiroTime){
            ...
            if (time.getPontuacaoRodada() == null){
                ...
                SpannableStringBuilder pontuacaoFormatada = new SpannableStringBuilder("C$ "+formatoCartoletas.format(time.getPatrimonio()));
            ...
            if (position == 0){
                ...
                String diferencaCalculada = "-"+formatoCartoletas.format(cartoletasPrimeiroTime-time.getPatrimonio());
