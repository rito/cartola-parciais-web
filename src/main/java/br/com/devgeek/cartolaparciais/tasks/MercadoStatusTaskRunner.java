 ...
 */
@Component
public class MercadoStatusTaskRunner {

    private static final int TEN_SECONDS = 10000;
    private static final String FIREBASE_DATABASE_REFERENCE = "mercado/status";
    private static final String URL = "https://api.cartolafc.globo.com/mercado/status";
    private static final SimpleDateFormat dateFormat = new SimpleDateFormat("HH:mm:ss");

    private FirebaseService firebaseService;

    private static Map<String, Object> mercadoStatus = null;

    @Autowired
    public MercadoStatusTaskRunner(FirebaseService firebaseService){
        ...
        try {
            ...
            if (apiReturn.getStatusCode() == HttpURLConnection.HTTP_OK && !apiReturn.getResponse().equals("null")){
                ...
                mercadoStatus = new Gson().fromJson(apiReturn.getResponse(), new TypeToken<HashMap<String, Object>>(){}.getType());
            }
    ...
    public void buscarDadosApiCartolaFC_mercadoStatus(){

        long startTime = System.currentTimeMillis();
        String task = "MercadoStatusTaskRunner - buscarDadosApiCartolaFC_mercadoStatus |" + dateFormat.format(new Date(startTime)) + "| -> ";
        ...
        try {
            ...
            if (apiReturn.getStatusCode() == HttpURLConnection.HTTP_OK){
                ...
                if (!mapsAreEqual(response, mercadoStatus)){
                ...
                System.out.println("MercadoStatusTaskRunner - buscarDadosApiCartolaFC_mercadoStatus | StatusCode -> " + apiReturn.getStatusCode() + " |");
            }

            System.out.println(task + String.valueOf(System.currentTimeMillis() - startTime) + " ms");

        } catch (Exception e){
            System.out.println("MercadoStatusTaskRunner - buscarDadosApiCartolaFC_mercadoStatus | " + dateFormat.format(System.currentTimeMillis()) + "|");
            e.printStackTrace();
        }
    }
}